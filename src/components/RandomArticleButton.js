...
const RandomArticleButton = ({ dispatchFn }) => {
  ...
  const getRandomWikiArticle = async () => {
    const { data } = await axios.get(
      `https://es.wikipedia.org/w/api.php?origin=%2A&action=query&format=json&generator=random&grnnamespace=0&grnlimit=5&prop=linkshere&lhnamespace=0&lhlimit=500&lhshow=%21redirect&lhprop=pageid`
    )
  ...
  const randomHandler = async (fn) => {
    const article = await getRandomWikiArticle().catch((e) =>
      console.error(`No se pudo obtener información de la wiki: ${e.message}`)
    )
  ...
  return (
    <RandomButton onClick={onClickHander} type="button">
      <VisuallyHidden>Seleccionár artículo al azar</VisuallyHidden>
